 ...
 *       unique.
 */
#define SYSX_MAKE_VERSION(x,y,z) \
  (((x)<<16) + ((y)<<8) + (z))
...
# define SYSX_IMPL_DIAG_DO_PRAGMA_(x) \
    _Pragma( SYSX_IMPL_STRINGIFY_(x) )
# define SYSX_IMPL_DIAG_PRAGMA_(x) \
    SYSX_IMPL_DIAG_DO_PRAGMA_(GCC diagnostic x)

# if defined(SYSX_CLANG_) || (SYSX_GCC_ >= SYSX_MAKE_VERSION(4,6,0))

#   define SYSX_DIAG_OFF_(x) \
      SYSX_IMPL_DIAG_PRAGMA_(push) \
      SYSX_IMPL_DIAG_PRAGMA_(ignored \
        SYSX_IMPL_STRINGIFY_(SYSX_IMPL_CONCAT_(-W,x)))

#   define SYSX_DIAG_ON_(x) \
      SYSX_IMPL_DIAG_PRAGMA_(pop)
...
#   define SYSX_DIAG_ON_(x) \
      SYSX_IMPL_DIAG_PRAGMA_(warning \
        SYSX_IMPL_STRINGIFY_(SYSX_IMPL_CONCAT_(-W,x)))
...
/* disable debug in case of a synthesis build */
#if defined( SYSX_SYNTHESIS_ )
#define NDEBUG
#endif
...
#ifndef SYSX_OVERRIDE
#if __cplusplus >= 201103L
#define SYSX_OVERRIDE override
#else
#define SYSX_OVERRIDE
#endif
#endif
 ...
 * \hideinitializer
 */
#define SYSX_IMPL_CONCAT_EVEN_MORE_DEFERRED_( First, Second ) \
  First ## Second
 ...
 *
 */
#define SYSX_IMPL_STRINGIFY_( Arg ) \
  SYSX_IMPL_STRINGIFY_DEFERRED_( Arg )
 ...
 * \hideinitializer
 */
#define SYSX_ANONYMOUS_VARIABLE( seed ) \
  SYSX_IMPL_CONCAT_( seed, __LINE__ )
